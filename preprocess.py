...
def preprocess_celeba(args):
    ...
    # Female Eyeglasses
    if args.config == 'female_glasses':
        for line in lines[2:]:
            line = line.split()
            if int(line[16]) == 1 and int(line[21]) == -1:
                allA.append(line[0])
            elif int(line[16]) == -1 and int(line[21]) == -1:
                allB.append(line[0])

    if args.config == 'male_glasses':
        for line in lines[2:]:
            line = line.split()
            if int(line[16]) == 1 and int(line[21]) == 1:
                allA.append(line[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="beard", help="configs available: glasses, mouth, beard")
